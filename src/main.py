...
from common_variables import win, color_selection_year, font_btn, color_text, color_btn, rankings_data, font_text, color_canvas, songs_text_data
...
class Main(tk.Frame):
    def __init__(self, container):
        ...
        # Ranking frame, Song text frame and Music player instances
    ...
    def song_choice(self):     
        ...
        if self.music_player_instance.playing_state == True:
            self.music_player_instance.stop()
    ...
    def show_ranking(self, event):
        rank_frame_center = int(win.winfo_screenwidth() / 4.5)
        rank_entries_left = int(win.winfo_screenwidth() / 55)
        ...
        rank_canvas.create_window(rank_frame_center, 20, window = self.ranking_instance.rank_title, anchor = "n")
        ...
        for canzone in rankings_data[self.year_chosen]:
            ...
            rank_canvas.create_window(rank_entries_left, pos_y, anchor = 'w', window = self.curRad)
            pos_y = pos_y + 30
