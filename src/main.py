...
class Main(tk.Frame):
    def __init__(self, container):
        ...
        self.main_frame = tk.Frame(common_variables.win, bg = common_variables.color_selection_year[0])
    ...
    def __create_widgets(self):
        ...
        select_year.configure(font = common_variables.font_btn)
        self.main_frame.pack(expand = "True", fill = "both")
        select_year.place(relheight = 0.05, relwidth = 0.15, relx = 0.175, rely = 0.04)
        
		# button that displays song's text inside frame
        self.text_button = tk.Button(self.main_frame, text = "Show text", font = common_variables.font_btn, fg = common_variables.color_text, bg = common_variables.color_btn, command = self.show_text, cursor = "hand2", state = "disabled")
        self.text_button.config(highlightthickness = 0, activebackground = common_variables.color_btn, activeforeground = common_variables.color_text)
        ...
        self.video_button = tk.Button(self.main_frame, text = "Go to video", font = common_variables.font_btn, fg = common_variables.color_text, bg = common_variables.color_btn, command = self.show_video, cursor = "hand2", state = "disabled")
        self.video_button.config(highlightthickness = 0, activebackground = common_variables.color_btn, activeforeground = common_variables.color_text)
    ...
    def show_ranking(self, event):
        ...
        if self.year_chosen == "2022":
            self.main_frame.configure(bg = common_variables.color_selection_year[0])
            self.logo_label.configure(image = logo_sanremo_2022, bg = common_variables.color_selection_year[0])
        elif self.year_chosen == "2021":
            self.main_frame.configure(bg = common_variables.color_selection_year[1])
            self.logo_label.configure(image = logo_sanremo_2021, bg = common_variables.color_selection_year[1])
        elif self.year_chosen == "2020":
            self.main_frame.configure(bg = common_variables.color_selection_year[2])
            self.logo_label.configure(image = logo_sanremo_2020, bg = common_variables.color_selection_year[2])
        ...
        pos_y = 90
        for canzone in common_variables.rankings_data[self.year_chosen]:
            dati_canzone = str(posizione_classifica) + " \"" + canzone['title'] + "\" " + canzone['singer']
            self.curRad = tk.Radiobutton(rank_frame, text = dati_canzone, font = common_variables.font_text, variable = self.radVar, value = posizione_classifica - 1, command = self.song_choice, bg = common_variables.color_canvas, cursor = "hand2")
            self.curRad.config(highlightthickness = 0, activebackground = common_variables.color_canvas, activeforeground = "#46ebfc", selectcolor = common_variables.color_canvas, fg = common_variables.color_text)
    ...
    def show_text(self):
        ...
        song_title = common_variables.rankings_data[self.year_chosen][num_canzone_scelta]['title']
        ...
        canzone = common_variables.songs_text_data[self.year_chosen][num_canzone_scelta]["song_text"]
    ...
    def show_video(self):
        num_canzone_scelta = self.song_choice()
        canzoni = common_variables.rankings_data[self.year_chosen]
        webbrowser.open_new(canzoni[num_canzone_scelta]['url'])
    
if __name__ == "__main__":
    main = Main(common_variables.win)
    main.mainloop()